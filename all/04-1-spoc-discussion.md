...
寄存器访问速度快，由硬件实现，但是空间小。高速缓存速度比较快，需要硬件支持，空间也比较小。内存比外存快，但空间比他小。外存最慢，空间最大。

2. 如何理解计算机中的存储层次结构所的理想状态是“均衡繁忙”状态？

各种存储介质都处于繁忙状态，但不会造成系统瓶颈。
...
覆盖技术是指划分程序功能模块，不同时执行的模块会共享一块内存。主要工作是划分功能模块，确定共享关系。

2. 什么是交换技术？覆盖与交换有什么不同？

交换是将程序整个地址空间换入换出。覆盖划分程序，交换将程序整体换入换出。

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

覆盖已经不需要了，交换还在一些系统中使用。

4. 如何分析内核模块间的依赖关系？

按模块调用关系建立关系树，将不会同时执行的分为一组。
...
虚拟储存是指只把当前指令执行所需要的部分的进程地址空间内容放到物理内存中。它不连续，由操作系统自己执行，仅仅交换部分程序，提供大的虚拟内存。
...
### 8.5 虚拟页式存储
 ...
 在页式储存的基础上，实现请求调页和页面置换。缺页中断处理的功能是从物理内存中将相应的页面调入内存中。
...
需要增加驻留位，修改位，访问位，保护位等。
 ...
 虚拟页式储存有内外存之间的内容交换，比页式储存多了调页和页面置换。

### 8.6 缺页异常

1. 缺页异常的处理流程？

首先在外存中查找需要访问的页面，备份修改过的页面，页面换入，修改页表项驻留位，重新执行指令。
